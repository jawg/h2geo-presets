...
const processImageField = function(presetName, field) {
  function cleanUri(uri) {
    ...
    switch (prefix) {
      ...
      case 'res':
        return `https://jawg.github.io/h2geo-presets/resources/${presetName}/${imageName}`;
      case 'http':
      case 'https':
        return uri;
      default:
  ...
  if (typeof field === 'string') {
    ...
    console.error('Unknown field type for an image: ' + field);
    throw new Error('Unknown field type for an image: ' + field);
  }
};
 ...
 */
var postProcessCleanup = function(presetName, parsed) {
  ...
  for (var i in parsed.groups) {
    ...
    if (typeof group.icon != 'undefined') {
      group.icon = processImageField(presetName, group.icon);
    }
    console.log("-----------------------------------------------------")
    ...
    for (var j in group.items) {
      ...
      if (typeof item.icon != 'undefined') {
        item.icon = processImageField(presetName, item.icon);
      }
      ...
      for (var k in item.tags) {
        ...
        if (typeof tag.images != 'undefined') {
          tag.images = processImageField(presetName, tag.images);
        }
      }
    }
  }
